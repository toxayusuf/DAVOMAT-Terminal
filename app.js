(() => {
'use strict';
const $=s=>document.querySelector(s), VERSION='0.9.0', CFG='davomat-terminal-config-v1', Q='davomat-offline-queue-v1';
const MODEL='https://cdn.jsdelivr.net/npm/@vladmandic/human@3.3.6/models/';
const S={cfg:null,human:null,stream:null,profiles:[],employees:new Map(),thr:{match:.62,live:.55,real:.55},samples:7,pending:null,mode:null,busy:false,running:false,hit:null,hits:0,timer:null,autoEnrollCode:null,launchEnrollCode:null,launchBackend:null,bridgeFrame:null,bridgeReady:false,bridgeInit:null,bridgeNonce:null,bridgePending:new Map()};
const msg={DEVICE_AUTH_FAILED:'Қурилма рухсати рад этилди',EMPLOYEE_NOT_FOUND:'Ходим топилмади',FACE_PROOF_LOW:'Юз текширувидан ўтмади',EVENT_TYPE_MISMATCH:'Нотўғри тугма танланди',ALREADY_MARKED:'Белги жуда тез такрорланди',NETWORK_ERROR:'Интернет билан алоқа йўқ',BRIDGE_TIMEOUT:'DAVOMAT сервер мости жавоб бермади',BRIDGE_ERROR:'DAVOMAT сервер мостида хато',TIMEOUT:'Сервер жавоб бермади'};
...
function show(id){['setupView','idleView','cameraView','resultView'].forEach(x=>$('#'+x).classList.toggle('hidden',x!==id));}
...
function launchParams(){const p=new URLSearchParams(location.search);return {backend:(p.get('backend')||'').trim(),pair:(p.get('pair')||'').trim(),enroll:(p.get('enroll')||'').trim()}}
...
function bridgeMessage(ev){
  const m=ev.data||{};
  if(!S.bridgeFrame||ev.source!==S.bridgeFrame.contentWindow||m.channel!=='DAVOMAT_BRIDGE'||m.nonce!==S.bridgeNonce)return;
  if(m.type==='ready'){
    S.bridgeReady=true;
...
async function bridgeCall(api,data={},timeout=30000){
  ...
  return new Promise((resolve,reject)=>{
    ...
    try{S.bridgeFrame.contentWindow.postMessage({channel:'DAVOMAT_BRIDGE',nonce:S.bridgeNonce,id,request},'*')}
...
async function initHuman(){if(S.human)return;S.human=new Human.Human({backend:'webgl',modelBasePath:MODEL,cacheSensitivity:.7,face:{enabled:true,detector:{rotation:true,maxDetected:1},mesh:{enabled:true},description:{enabled:true},iris:{enabled:true},emotion:{enabled:false},antispoof:{enabled:true},liveness:{enabled:true}},body:{enabled:false},hand:{enabled:false},object:{enabled:false}});await S.human.load();await S.human.warmup()}
...
async function boot(){const lp=launchParams();try{importHash();S.cfg=loadCfg();if(!valid(S.cfg)){show('setupView');$('#setupError').textContent='Терминал ҳали уланмаган. Администратор орқали бир марта уланг.';return}show('idleView');await initBridge();await Promise.all([initHuman(),bootstrap()]);const enrollCode=lp.enroll||'';if(enrollCode)await loadEnrollmentByCode(enrollCode);else await pending();syncQueue();setInterval(pending,4000);setInterval(()=>navigator.onLine&&syncQueue(),30000)}catch(e){console.error(e);await stopCamera();show('setupView');$('#setupError').textContent='Терминал серверга уланмади. «Қайта уриниш» ни босинг.';const detail=$('#setupDetail');if(detail)detail.textContent=friendly(e)}}
